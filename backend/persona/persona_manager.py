...
class PersonaManager:
    def __init__(self, characters_dir: Path | None = None) -> None:
        if characters_dir:
            ...
            if IS_FROZEN:
                self.characters_dir = RESOURCE_BASE / "backend" / "persona" / "characters"
            else:
                self.characters_dir = PROJECT_ROOT / "backend" / "persona" / "characters"
    ...
    def load_persona(self, name: str) -> dict:
        ...
        # Fallback to main project config/characters if not in backend
        if not path.exists():
