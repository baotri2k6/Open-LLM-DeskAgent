...
function liveTargets() {
    return electron_1.BrowserWindow.getAllWindows().filter(win => win && !win.isDestroyed());
}
...
function setLipsync(active) {
    ...
    (0, websocket_server_1.broadcast)("lipsync", active);
}
function setEmotion(emotion) {
    sendToTargets("set:emotion", emotion);
    (0, websocket_server_1.broadcast)("emotion", emotion);
}
...
function emitAssistantResponse(response) {
    ...
    if (text) {
        ...
        (0, websocket_server_1.broadcast)("chat_chunk", text);
    }
    if (response?.audio_url) {
        ...
        (0, websocket_server_1.broadcast)("tts_audio", {
            url: response.audio_url,
            duration_ms: response.duration_ms || 0,
        });
    }
    sendToTargets("chat:done", text);
    (0, websocket_server_1.broadcast)("chat_done", text);
    if (!response?.audio_url)
        setLipsync(false);
}
...
function registerAiIpc(ipcMain, windows) {
    ...
    ipcMain.handle("ai:chat", async (_e, { text, image, context }) => {
        return new Promise((resolve) => {
            ...
            let audioUrl = null;
            let durationMs = 0;
            requestStream("POST", "/chat", { text, image, context }, chunk => {
                if (chunk.type === "start") {
                    setEmotion(chunk.emotion || "normal");
                    if (chunk.motion)
                        setLipsync(chunk.motion === "thinking");
                    (0, websocket_server_1.broadcast)("start", { emotion: chunk.emotion || "normal", motion: chunk.motion });
                }
                    ...
                    if (chunk.thought) {
                        ...
                        (0, websocket_server_1.broadcast)("thought_chunk", chunk.text);
                    }
                    else {
                        sendToTargets("chat:chunk", chunk.text);
                        fullText += chunk.text;
                        (0, websocket_server_1.broadcast)("chat_chunk", chunk.text);
                    }
                }
                ...
                (0, websocket_server_1.broadcast)("chat_done", fullText);
                ...
                (0, websocket_server_1.broadcast)("chat_done", `Có lỗi xảy ra: ${err.message}`);
        ...
        try {
            ...
            if (key === "app.avatarScale") {
                ...
                if (avatarWin && !avatarWin.isDestroyed()) {
                    ...
                    const newW = Math.min(sw, Math.round(AVATAR_WINDOW_WIDTH * scale));
                    const newH = Math.min(sh, Math.round(AVATAR_WINDOW_HEIGHT * scale));
