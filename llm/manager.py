...
class LLMService:
    ...
    async def chat_stream(self, message: str | list, context: dict | None = None, image: str | None = None):
        ...
        try:
            import random
            spontaneity_val = getattr(self._persona, "spontaneity", 0.3)
            if random.random() < spontaneity_val:
                extra = random.choice([
                ...
                logger.info("Spontaneity triggered: adding spontaneous instruction: %s", extra)
        except Exception as e:
            logger.warning("Failed to inject spontaneity: %s", e)
            ...
            try:
                ...
                if matched_skills:
                    for ms in matched_skills:
                        messages.append({
                            "role": "system",
                            "content": f"=== CHI TIẾT HƯỚNG DẪN KỸ NĂNG: {ms['name']} ===\n{ms['content']}"
                        })
                        logger.info("Dynamically injected full content for skill: %s", ms['name'])
            except Exception as e:
                logger.error(f"Failed to dynamically match and inject skills: {e}")
