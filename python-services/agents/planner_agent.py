...
class PlannerAgent:
    ...
    def detect_intent(self, text: str) -> dict[str, Any]:
        ...
        if m and not re.search(r"(http|www|\.com|trang)", t):
            return {"name": "open_app", "app": m.group(1).strip()}
        ...
        # Tìm kiếm web
        m = re.search(r"(?:tìm|search|tìm kiếm|google)\s+(.+)", t)
        if m:
            return {"name": "web_search", "query": m.group(1).strip()}

        # Đọc clipboard
        if re.search(r"(clipboard|copy|paste|dán|sao chép)", t):
            return {"name": "clipboard_read"}

        # Đọc file
        m = re.search(r"(?:đọc|read|mở file|open file)\s+(.+)", t)
        if m:
            return {"name": "read_file", "path": m.group(1).strip()}

        # Chụp màn hình / nhìn màn hình
        if re.search(r"(màn hình|screenshot|chụp|nhìn|ocr|man hinh)", t):
            return {"name": "screen_read"}
