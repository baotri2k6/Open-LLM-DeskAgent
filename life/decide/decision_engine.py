...
class DecisionEngine:
    ...
    def decide(self, context: LifeContext) -> Decision:
        ...
        # ── 0. Critical hardware alerts (J.A.R.V.I.S. Bypass Cooldown) ──────
        is_cpu_crit = getattr(context, "cpu_usage", 0.0) > 85.0
        is_ram_crit = getattr(context, "ram_usage", 0.0) > 90.0
        is_disk_crit = getattr(context, "disk_usage", 0.0) > 90.0
        
        if is_cpu_crit or is_ram_crit or is_disk_crit:
            alert_type = "cpu" if is_cpu_crit else ("ram" if is_ram_crit else "disk")
            val = getattr(context, f"{alert_type}_usage", 0.0)
            self._last_proactive_time = now
            return Decision(
                should_act=True,
                action_type=f"proactive_hardware_alert_{alert_type}",
                message_hint=f"Cảnh báo hệ thống: {alert_type.upper()} đang quá tải ở mức {val:.1f}%!",
                next_check_seconds=30.0,
            )
