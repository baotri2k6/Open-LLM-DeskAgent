...
class LifeContext:
    ...
    # Hardware Telemetry (J.A.R.V.I.S.)
    cpu_usage: float   = 0.0   # %
    ram_usage: float   = 0.0   # %
...
class LifeObserver:
    ...
    def observe(
        ...
        try:
            import psutil
            cpu_val = float(psutil.cpu_percent())
            ram_val = float(psutil.virtual_memory().percent)
            disk_val = float(psutil.disk_usage("/").percent)
        except Exception:
            pass
    ...
    def _compute_focus_index(
        ...
        if any(token in app_text for token in ["code", "terminal", "powershell", "pycharm", "intellij"]):
            score += 0.25
        if any(token in screen for token in ["traceback", "error:", "def ", "class ", "import ", "npm ", "pnpm ", "pytest"]):
            score += 0.2
        if idle_seconds < 120:
            score += 0.1
