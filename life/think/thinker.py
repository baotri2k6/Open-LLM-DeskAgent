...
class Thinker:
    ...
    def think(self, context: Any) -> Dict[str, Any]:
        ...
        if is_cpu_crit or is_ram_crit or is_disk_crit:
            ...
            val = getattr(context, f"{alert_type.lower()}_usage", 0.0)
            thoughts.append(f"Cảnh báo hệ thống: {alert_type} đang quá tải ở mức {val:.1f}%. Mình phải thông báo khẩn cấp cho người dùng biết!")
            proposed_intent = f"hardware_alert_{alert_type.lower()}"
            category = "proactive"
            priority = 5
            stay_silent = False
