...
class LifeLearner:
    ...
    def learn_cycle_lessons(self, context: Any, decision: Any, success: bool) -> None:
        ...
        if hour is not None:
            # Ghi nhận dữ liệu thói quen để PatternLearner học tập
            try:
                from learning.pattern_learning.pattern_learner import pattern_learner
                pattern_learner.record_activity(hour, activity, active_window)
            except Exception as pe:
                logger.warning("LifeLearner failed to record pattern activity: %s", pe)
        ...
        # Định kỳ chạy đúc kết tri thức (KnowledgeDistiller)
        try:
            from learning.knowledge_distillation.knowledge_distiller import knowledge_distiller
            knowledge_distiller.distill_experiences()
        except Exception as de:
            logger.warning("LifeLearner failed to distill experiences: %s", de)
