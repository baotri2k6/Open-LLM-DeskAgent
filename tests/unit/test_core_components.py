...
def test_character_profile_spontaneity() -> None:
    ...
    assert p.spontaneity == 0.3
    
    # Test custom dict load
    custom_yaml_data = {
        "name": "CustomGirl",
        "description": "Test desk companion",
        "personality": {"cheerful": 0.8},
        "spontaneity": 0.75
    }
    p_custom = CharacterProfile.from_yaml(custom_yaml_data)
    assert p_custom.spontaneity == 0.75
    assert p_custom.name == "CustomGirl"

    # Test YAML fallback defaults
    p_fallback = CharacterProfile.from_yaml({"name": "NoSpontaneity"})
    assert p_fallback.spontaneity == 0.3
    
    # Test PersonaManager load
    mgr = PersonaManager()
    p_ice = mgr.get_character_profile("icegirl")
    assert p_ice.spontaneity == 0.65
    
    p_hi = mgr.get_character_profile("hiyori")
    assert p_hi.spontaneity == 0.85
