...
@pytest.mark.anyio
async def test_agent_coordinator_error_and_timeout_isolation() -> None:
    from agents.coordinator.agent_coordinator import agent_coordinator
    from unittest.mock import patch

    # Mock run_subagent to simulate success, crash, and hang
    async def mock_run_subagent(task: str, focus_files=None):
        if "Success" in task:
            return {"success": True, "summary": "Success Result"}
        elif "Crash" in task:
            raise RuntimeError("Crashed")
        elif "Hanging" in task:
            import asyncio
            await asyncio.sleep(5)
            return {"success": True, "summary": "Should have timed out"}
        return {"success": False, "error": "Unknown task"}

    with patch("agents.subagent_service.run_subagent", side_effect=mock_run_subagent):
        subtasks = [
            {"task": "Success Task", "timeout": 0.5},
            {"task": "Crash Task", "timeout": 0.5},
            {"task": "Hanging Task", "timeout": 0.1}
        ]
        ...
        # 1. Success subagent
        assert results[0]["success"] is True
        assert results[0]["summary"] == "Success Result"
